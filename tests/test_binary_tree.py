...
def test_insert():
    ...
    assert tree.inorder_traversal() == [
        ...
        10,
        15,
    ], "Inorder traversal after insertions is incorrect."
...
def test_search():
    ...
    assert (
        tree.search(20) is False
    ), "Search for non-existing element 20 failed."
...
def test_inorder_traversal():
    ...
    assert tree.inorder_traversal() == [
        ...
        10,
        15,
    ], "Inorder traversal is incorrect."
...
def test_empty_tree_operations():
    ...
    ), "Inorder traversal of an empty tree should be an empty list."
    assert (
        tree.search(10) is False
    ), "Search in an empty tree should return False."
    assert (
        tree.find_min() is None
    ), "Minimum value in an empty tree should be None."
    assert (
        tree.find_max() is None
    ), "Maximum value in an empty tree should be None."
